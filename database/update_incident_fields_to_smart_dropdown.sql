-- Migration: Update Safety Incident Description Fields to Smart Dropdown
-- This updates all safety incident description fields to use the new smart-incident-select type

-- Update all existing safety incident description questions to use smart-incident-select type
...
WHERE 
  pillar = 'safety' 
  AND question_id LIKE '%-incident-%-description'
  AND question_type = 'textarea';

-- Update question text to be more concise since the smart dropdown will handle the details
UPDATE pillar_questions 
SET 
  question_text = CASE 
