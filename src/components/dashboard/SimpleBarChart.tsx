...
export const SimpleBarChart = ({ data, title, pillar }: SimpleBarChartProps) => {
  ...
  return (
    <div className="bg-card/50 backdrop-blur-sm border-border/50 rounded-xl p-6 shadow-lg">
      ...
      <div className="h-32 flex items-end justify-between gap-1 relative mb-4">
        {/* Grid lines */}
        ...
        {/* Y-axis labels */}
        <div className="absolute -left-8 h-full flex flex-col justify-between text-xs text-muted-foreground">
          {[100, 75, 50, 25, 0].map((value) => (
        ...
        {data.map((item, index) => {
          ...
          return (
            <div key={index} className="flex-1 relative flex flex-col justify-end min-w-[40px]">
              {/* Target line */}
              <div 
                className="absolute w-full border-t-2 border-amber-400 z-10 opacity-70"
                style={{ bottom: `${targetHeight}%` }}
              />
              
              {/* Value label above bar */}
              <motion.div 
                className="absolute -top-6 left-1/2 transform -translate-x-1/2 text-xs font-medium text-foreground bg-background/80 px-1 rounded"
              ...
              {/* Bar */}
              <motion.div
                className={`w-full rounded-t-lg shadow-lg border-t-2 ${
                  isOverTarget 
              ...
              {/* Day label */}
              <div className="text-sm font-medium text-center text-foreground mt-2">
                {item.day}
              </div>
            </div>
          );
